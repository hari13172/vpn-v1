    ...
    return (
        <div className='lg:w-full h-full flex bg-zinc-900 '>
            ...
            <div className='w-full rounded-md h-full overflow-y-auto'>
                ...
                <div className='px-2'>
                    <Button variant="outline" size="sm" className='w-full' onClick={() => setUserForm(e => !e)}>
                        <i className="pi pi-plus mr-2"></i>Create New Peer
                    </Button>
                    <Dialog open={userForm} onOpenChange={setUserForm}>
                        <div className="dialog-header">Create New VPN</div>
                        <div className='flex flex-col gap-4'>
                            <div className="flex flex-col gap-2">
                                <Input placeholder='Username' value={user} name='username' disabled />
                                <small>Enter username to Create VPN Peer.</small>
                            </div>
                            <div className="flex flex-col gap-2">
                                <Input placeholder='Device Name' value={DeviceName} onChange={(e) => setDeviceName(e.target.value)} />
                                <small>Enter Device Name for the VPN Peer.</small>
                            </div>
                            <Button variant="outline" size="sm" onClick={CreateVpn}>
                                {apiloading ? "Creating..." : "Create VPN"}
                            </Button>
                        </div>
                    </Dialog>
                    <div className='w-full lg:grid lg:grid-cols-4 lg:gap-4 m-auto grid grid-cols-1 gap-4 mt-4 md:grid md:grid-cols-2 '>
                        {loading
                            ? Array.from({ length: users.length || 2 }).map((_, idx) => <div key={idx}></div>)
                            : users.map((user: any) => (
                                <div key={user.ipAddress} className=' flex flex-col gap-3 border rounded-md p-2 p-ripple cursor-pointer bg-black'
                                    style={{
                                        borderColor: user.ipStatus === 'online' ? 'var(--green-700)' : 'var(--red-700)',
                                    }}
                                    onClick={() => {
                                        setUsersIp(user);
                                        setVisible(true);
                                ...
                                >
                                    {/* <Ripple pt={{ root: { style: { background: user.ipStatus === 'online' ? 'var(--green-700)' : 'var(--red-700)' } } }} /> */}
                                    <div className='flex justify-between items-end w-full'>
                                        <div>
                                            <p className='font-semibold text-base'>{user.deviceName.length > 20 ? `${user.deviceName.substring(0, 20)}...` : user.deviceName}</p>
                                            <div className='flex items-center gap-2'>
                                                <p className='text-xs'>{user.ipAddress}</p>
                                                <Input value={user.ipStatus} className='p-0 h-5 rounded-xl px-2 text-[10px] font-semibold'
                                                    style={{
                                                        backgroundColor: user.ipStatus === 'online' ? 'var(--green-600)' : 'var(--red-600)',
                                                    }} readOnly />
                                            </div>
                                        </div>

                                        <Input value={formatTimeAgo(user.createdAt)} className='p-0 h-5 rounded-xl px-1 text-[8px]'  />
                                        {/* <Chip label={formatTimeAgo(user.createdAt)} className='p-0 h-5 rounded-xl px-1 text-[8px]' /> */}


                                    </div>
                                    <div className='lg:flex lg:justify-between lg:items-center w-full pt-2'>
                                        <div className='lg:flex flex-col gap-1 lg:w-full'>
                                            <div className='flex justify-between text-xs'>
                                                <span>Endpoint:</span>
                                                <span>{user.transfer.endpoint || '🔗'}</span>
                                            </div>
                                            <div className='flex justify-between text-xs'>
                                                <span>Allowed IPs:</span>
                                                <span>{user.transfer.allowed_ips || '❌'}</span>
                                            </div>
                                            <div className='flex justify-between text-xs'>
                                                <span>Latest Handshake:</span>
                                                <span>{user.transfer.latest_handshake || '🤝'}</span>
                                            </div>
                                            <div className='flex justify-between text-xs'>
                                                <span>Received:</span>
                                                <span>{user.transfer.transfer_received || '⬇️'}</span>
                                            </div>
                                            <div className='flex justify-between text-xs'>
                                                <span>Sent:</span>
                                                <span>{user.transfer.transfer_sent || '⬆️'}</span>
                                            </div>
                                        </div>
                                    </div>
                                </div>
